...
export function practice(videoRef, targetLetter, detectionActive = ref(true)) {
  const isMatch = ref(false); // whether detected letter matches target
  const detectedLabel = ref(null); // predicted letter from model
  const detectionConfidence = ref(0); // confidence of prediction
  const noHandDetected = ref(false); // true if no hand is visible
  ...
  let holdCount = 0;
  let lastSeenLabel = null; // last predicted  label
  ...
  // get target letter
  const getTarget = () =>
    isRef(targetLetter) ? targetLetter.value : targetLetter;
      ...
      interval = setInterval(async () => {
        ...
        try {
          ...
          if (data.confidence >= 85) {
            ...
            //register letter only after 3 consistent predictions
