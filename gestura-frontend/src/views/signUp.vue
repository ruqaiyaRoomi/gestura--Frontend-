...
const validateConfirmPass = computed(() => {
  return (
    confirmPass.value === userInfo.value.password &&
...
const showPassError = computed(() => {
  return userInfo.value.password.length > 0 && !validatePass.value;
});
...
const showNameError = computed(() => {
  return (
    (userInfo.value.lastName.length > 0 ||
      userInfo.value.firstName.length > 0) &&
    !validateName.value
  );
});
